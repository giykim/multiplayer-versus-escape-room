...
## Dungeon3D - Manages active 3D dungeon instance
## Handles room loading/unloading, player navigation, and progress tracking
## Same interface as 2D Dungeon.gd
...
const BASE_ROOM_SCENE_3D: String = "res://src/dungeon3d/Room3D.tscn"

# Room spacing on X axis (10m apart)
...
var loaded_rooms: Dictionary = {}  # room_index -> Room3D node
...
func generate_dungeon(seed_value: int = -1) -> void:
	...
	if not generator.validate_layout(layout):
		push_error("[Dungeon3D] Generated layout failed validation")
		return
	...
	print("[Dungeon3D] Generated dungeon with %d rooms" % layout.room_count)
	dungeon_generated.emit(layout.room_count)


## Get the current room
func get_current_room() -> Room3D:
...
func transition_to_room(room_index: int, player_id: int = 1, entry_direction: String = "") -> bool:
	...
	if current_room:
		...
		if not current_room.can_use_door(direction):
			print("[Dungeon3D] Door is locked, cannot transition")
			return false
	...
	if not new_room:
		push_error("[Dungeon3D] Failed to load room %d" % room_index)
		return false
	...
	print("[Dungeon3D] Player %d transitioned from room %d to room %d" % [
...
func _load_room(room_index: int) -> Room3D:
	...
	var scene_path = ROOM_SCENES_3D.get(room_data.type, BASE_ROOM_SCENE_3D)

	# Check if scene exists, fall back to base
	if not ResourceLoader.exists(scene_path):
		print("[Dungeon3D] Scene not found: %s, using base room" % scene_path)
		scene_path = BASE_ROOM_SCENE_3D

	var room_scene = load(scene_path)
	if not room_scene:
		push_error("[Dungeon3D] Failed to load room scene: %s" % scene_path)
		return null

	var room = room_scene.instantiate() as Room3D
	if not room:
	...
	print("[Dungeon3D] Loaded room %d (%s) at position %s" % [
		room_index, DungeonGenerator.get_room_type_name(room_data.type), room.position
	])
...
func _unload_room(room_index: int) -> void:
	...
	print("[Dungeon3D] Unloaded room %d" % room_index)
...
## Get all currently loaded room indices
func get_loaded_room_indices() -> Array[int]:
	var indices: Array[int] = []
	for key in loaded_rooms.keys():
		indices.append(key)
	return indices


## Force activate a specific room
func activate_room(room_index: int) -> void:
...
func _on_room_completed(room_index: int) -> void:
	print("[Dungeon3D] Room %d completed" % room_index)
...
func _on_player_reached_arena(player_id: int) -> void:
	...
	print("[Dungeon3D] Player %d reached arena in %.2f seconds" % [
...
func debug_print_layout() -> void:
	if not layout:
		print("[Dungeon3D] No layout generated")
		return

	print("=== Dungeon3D Layout (Seed: %d) ===" % layout.generation_seed)
	...
	print("Room Spacing: %.1fm on X axis" % ROOM_SPACING)
	print("")
	...
	for room in layout.rooms:
		...
		var loaded_info = " (LOADED)" if loaded_rooms.has(room.index) else ""

		print("  Room %d: %s (diff: %d)%s%s" % [
			room.index, type_name, room.difficulty, puzzle_info, loaded_info
		])
		print("    Position: (%.1f, 0, 0)" % (room.index * ROOM_SPACING))
